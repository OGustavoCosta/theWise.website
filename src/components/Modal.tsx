...
function Modal({close, course, isActive}: IProps){
    ...
    return(
        <div onClick={close} className={`modal__background fixed top-0 left-0 w-full h-full px-[10vw] py-10 bg-[#00000090] flex justify-center items-center z-10 ${isActive && 'fadeIn'}`}>
            <section onClick={e => e.stopPropagation()} className={`modal w-full max-w-lg h-full max-h-[450px] md:max-h-[600px] bg-light border-4 border-light text-dark rounded-2xl relative grid overflow-hidden xmd:py-3 py-3 ${isActive && 'scaleUp'}`}>
