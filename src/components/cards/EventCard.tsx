...
function EventCard({event}: {event: IEvent}){
    const { banner, title, shortDescription, altBanner } = event
    ...
    return(
        ...
        <article className="card card--event w-full h-[512px] relative overflow-hidden rounded-4xl">
            <img className="card__banner w-full h-full object-cover transition-all duration-1000" src={banner} alt={altBanner}/>
